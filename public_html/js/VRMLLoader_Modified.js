				...
				if ( 'Transform' === data.nodeType || 'Group' === data.nodeType ) {
					...
					if ( 'Box' === data.nodeType ) {
						...
						for ( var i = 0, j = data.children.length; i < j; i++ ) {
							...
							var vec;
                                                        ...
                                                        //Handle USE declaration for coordinates.... This is a bit of a hack
                                                        if ( typeof child === 'string' ) {
                                                            if ( /USE/.exec( child ) ) {
                                                                var defineKey = /USE\s+?(\w+)/.exec( child )[ 1 ];
                                                                if (undefined == defines[defineKey]) {
                                                                        console.warn(defineKey + ' is not defined.');
                                                                } else {
                                                                    if ( /coord/.exec( child ) && defineKey ) {
                                                                        geometry = defines[ defineKey ].clone();
                                                                        ...
                                                                        break; //We've found the coordinates and there's nothing else to find
                                                                    }
                                                                }
                                                            }
                                                        }
								...
								for ( var k = 0, l = child.points.length; k < l; k++ ) {
									...
									geometry.vertices.push( vec );
                                                                ...
                                                                if ( /DEF/.exec( child.string ) ) {
                                                                        geometry.name = /DEF (\w+)/.exec( child.string )[ 1 ];
					...
					for ( var i = 0; i < data.children.length; i ++ ) {
						...
						var child = data.children[ i ];
                                                ...
                                                //Handle USE declaration for materials.... This is a bit of a hack
                                                ...
                                                if ( typeof child === 'string' ) {
                                                    if ( /USE/.exec( child ) ) {
                                                        ...
                                                        if (undefined == defines[defineKey]) {
                                                            ...
                                                            if ( /material/.exec( child ) && defineKey ) {
                                                                parent.material = defines[ defineKey ].clone();
                                                                break; //We've found the material and there's nothing else to find
                                                            }
                                                        }
                                                    }
                                                }        
						...
						if ( 'Material' === child.nodeType ) {
							...
							if ( /DEF/.exec( data.string ) ) {
								...
								defines[ material.name ] = material;
                                                        ...
                                                        //Record DEF on the Material
                                                        if ( /DEF/.exec( child.string ) ) {

								material.name = /DEF (\w+)/.exec( child.string )[ 1 ];
